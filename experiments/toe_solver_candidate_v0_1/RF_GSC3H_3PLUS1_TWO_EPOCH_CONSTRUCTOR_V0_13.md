...
\boxed{
S
\xrightarrow{\pi_{clock},\pi_{space},\pi_{match}}
(C,P,M),
\qquad
R(C)=R(P)=R(M)=H(S),
}
...
The source is an external observational archive, but the topology and packet construction remain model-derived.
...
observational/model-level E2E                    PASS PARENT
full physical 3+1 production capture            OPEN
physical production claim                       FALSE
canon allowed                                   FALSE
```

The mnemonic may guide the constructor. It cannot by itself promote model-derived geometry into physical-production evidence.
