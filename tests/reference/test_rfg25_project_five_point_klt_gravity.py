...
def bg_amplitude(ps,es,quartic_scale=1.0):
    @lru_cache(None)
    def mom(seq): return sum((ps[i] for i in seq),np.zeros(4))
    @lru_cache(None)
    def cur(seq):
    ...
    def num(seq):
        ...
        for k in range(1,L):
            X,Y=seq[:k],seq[k:]; JX,JY=cur(X),cur(Y); kX,kY=mom(X),mom(Y); jj=dot(JX,JY)
            out += math.sqrt(2)*(dot(kY,JX)*JY+0.5*kX*jj-dot(kX,JY)*JX-0.5*kY*jj)
        for i in range(1,L-1):
            for j in range(i+1,L):
                X,Y,Z=seq[:i],seq[i:j],seq[j:]; JX,JY,JZ=cur(X),cur(Y),cur(Z)
                out += quartic_scale*(dot(JX,JZ)*JY-0.5*dot(JX,JY)*JZ-0.5*dot(JY,JZ)*JX)
        return out
    return dot(num(tuple(range(len(ps)-1))),es[-1])

def A(ps,es,order,quartic_scale=1.0): return bg_amplitude([ps[i] for i in order],[es[i] for i in order],quartic_scale)
def sij(ps,i,j): return 2*dot(ps[i],ps[j])
def kernel(ps):
    s12,s13,s23=sij(ps,0,1),sij(ps,0,2),sij(ps,1,2)
    return np.array([[s12*(s13+s23),s12*s13],[s12*s13,s13*(s12+s23)]],float)
def klt(ps,eL,eR,qL=1.0,qR=1.0):
...
def test_project_five_point_klt_matrix_matches_two_term_form():
    ...
    for _ in range(150):
        ps=generate_2to3(rng); eL=polarizations(ps,rng); eR=polarizations(ps,rng)
        x,y=klt(ps,eL,eR),klt_two_term(ps,eL,eR)
        assert abs(x-y)<8e-11*max(1.0,abs(x),abs(y))

def test_project_five_point_gravity_ward_identity_in_left_copy():
    rng=np.random.default_rng(20260909)
    for _ in range(100):
        ps=generate_2to3(rng); eL=polarizations(ps,rng); eR=polarizations(ps,rng); scale=max(1.0,abs(klt(ps,eL,eR)))
        for leg in range(5):
            x=[e.copy() for e in eL]; x[leg]=ps[leg].copy()
            assert abs(klt(ps,x,eR))<2e-10*scale
...
def test_project_five_point_gravity_ward_identity_in_right_copy():
    ...
    for _ in range(100):
        ps=generate_2to3(rng); eL=polarizations(ps,rng); eR=polarizations(ps,rng); scale=max(1.0,abs(klt(ps,eL,eR)))
        for leg in range(5):
            x=[e.copy() for e in eR]; x[leg]=ps[leg].copy()
            assert abs(klt(ps,eL,x))<2e-10*scale
...
def test_project_klt_is_copy_exchange_symmetric():
    ...
    for _ in range(120):
        ...
        assert abs(a-b)<3e-10*max(1.0,abs(a),abs(b))

def test_quartic_contact_is_required_for_gravitational_ward_closure():
    rng=np.random.default_rng(20260912); witnessed=False
    for _ in range(20):
        ps=generate_2to3(rng); eL=polarizations(ps,rng); eR=polarizations(ps,rng)
        good=[]; bad=[]
        for leg in range(5):
            x=[e.copy() for e in eL]; x[leg]=ps[leg].copy()
            good.append(abs(klt(ps,x,eR,1.0,1.0))); bad.append(abs(klt(ps,x,eR,0.0,1.0)))
        assert max(good)<3e-9
        if max(bad)>1e-3: witnessed=True
    assert witnessed

def test_project_five_point_gravity_prefactor_is_reduced_scale_cubed():
    for mbar in [0.8,1.0,2.0,4.5]:
        kappa_g=2/mbar; kappa_E=1/(mbar*mbar)
        pref=(kappa_g/2)**3
        assert math.isclose(pref,1/mbar**3,rel_tol=1e-15)
        assert math.isclose(pref,kappa_E/mbar,rel_tol=1e-15)
