...
def ordered_amplitude(ps, es, order, **kw):
    return bg_amplitude([ps[i] for i in order], [es[i] for i in order], **kw)


def sij(ps, i, j):
    return 2.0 * dot(ps[i], ps[j])
...
def test_project_five_point_states_are_massless_conserved_and_transverse():
    ...
    for _ in range(120):
        ...
        es = polarizations(ps, rng)
        assert np.linalg.norm(sum(ps)) < 2e-13
        for p, e in zip(ps, es):
            assert abs(dot(p, p)) < 2e-13
            assert abs(dot(p, e)) < 2e-13
...
def test_project_five_point_all_leg_ward_identities():
    ...
    for _ in range(180):
        ps = generate_2to3(rng)
        es = polarizations(ps, rng)
        scale = max(1.0, abs(bg_amplitude(ps, es)))
        for leg in range(5):
            replaced = [e.copy() for e in es]
...
def test_project_five_point_fundamental_bcj_relation():
    ...
    for _ in range(220):
        ps = generate_2to3(rng)
        es = polarizations(ps, rng)
        a1 = ordered_amplitude(ps, es, [0, 1, 2, 3, 4])
        a2 = ordered_amplitude(ps, es, [0, 2, 1, 3, 4])
        a3 = ordered_amplitude(ps, es, [0, 2, 3, 1, 4])
...
def test_project_five_point_reflection_and_photon_decoupling():
    ...
    for _ in range(150):
        ps = generate_2to3(rng)
        es = polarizations(ps, rng)
        a = ordered_amplitude(ps, es, [0, 1, 2, 3, 4])
        ar = ordered_amplitude(ps, es, [4, 3, 2, 1, 0])
...
def test_quartic_normalization_is_required_for_five_point_ward_closure():
    ...
    for _ in range(30):
        ...
        for leg in range(5):
            replaced = [e.copy() for e in es]
            replaced[leg] = ps[leg].copy()
            good.append(abs(bg_amplitude(ps, replaced, quartic_scale=2.0)))
            bad.append(abs(bg_amplitude(ps, replaced, quartic_scale=0.0)))
        assert max(good) < 6e-10
        if max(bad) > 1e-3:
            witnessed = True
    assert witnessed
...
def test_project_five_point_coupling_power_is_g_cubed():
    ...
    ps = generate_2to3(rng)
    es = polarizations(ps, rng)
    stripped = bg_amplitude(ps, es)
    physical = G_YM**3 * stripped
