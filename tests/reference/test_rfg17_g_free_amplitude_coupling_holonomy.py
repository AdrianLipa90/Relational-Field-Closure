...
def project_core_prefactor_from_coupling(kappa_e):
    return kappa_e / 4.0
...
def test_horizon_reduced_scale_gives_same_physical_einstein_coupling():
...
def test_local_gamma_horizon_binding_closes_physical_coupling():
...
def test_project_core_prefactor_is_G_free_on_horizon_surface():
    rng = np.random.default_rng(20260903)
    for _ in range(100):
        MH = float(rng.uniform(0.3, 9.0))
        TH = float(rng.uniform(0.03, 2.0))
        core = float(rng.normal())
        kappa_e = coupling_from_horizon(MH, TH)
        a = project_core_prefactor_from_coupling(kappa_e) * core
        b = core / (4.0 * MH * TH)
        assert math.isclose(a, b, rel_tol=2e-15, abs_tol=2e-15)


def test_project_core_prefactor_is_G_free_on_local_carrier_surface():
    ...
    gamma = gamma_from_local(alpha, omega, MH, TH)
    core = -2.41
    local = project_core_prefactor_from_coupling(coupling_from_local(alpha, omega, gamma)) * core
    horizon = project_core_prefactor_from_coupling(coupling_from_horizon(MH, TH)) * core
...
def test_no_numerical_Newton_constant_is_required():
    ...
    prefactor = project_core_prefactor_from_coupling(kappa_e)
    assert kappa_e > 0.0 and math.isfinite(kappa_e)
    assert prefactor > 0.0 and math.isfinite(prefactor)
