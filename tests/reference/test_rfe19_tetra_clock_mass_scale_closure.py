...
def test_promoted_tir_shape_coefficient():
...
def test_kappa_rewrite_for_canonical_tir_kappa():
...
def test_physical_beta_directional_energy_after_scale_closure():
    beta = 0.24
    mass = 1.6
    e_plus = physical_directional_energy_natural(beta, +1, mass)
    e_minus = physical_directional_energy_natural(beta, -1, mass)
...
def test_physical_beta_branch_is_parity_conjugate():
    beta = 0.37
    assert physical_directional_energy_natural(beta, +1, 1.0) == pytest.approx(
...
def test_physical_beta_domain_fails_closed(beta):
    with pytest.raises(TetraClockMassScaleClosureError):
        physical_directional_energy_natural(beta, +1, 1.0)
